...
def main(args):
    ...
    with torch.no_grad():
        ...
        for idx, X in pbar: 
            ...
            # else:
            #     pred = model(X)
            myPredSeg, myPredClass = model(X)
            # segmentationMaps.append(myPredSeg)
            myPredSigmoid = nn.Softmax(dim=1)(myPredClass)
            # print(np.partition(myPredSigmoid.ravel(), -2)[-2:])
            # print(myPredSigmoid)
            # print(Y)
            segmentationMaps.append(myPredSeg)
            ...
            probs.append(myPredClass.cpu().numpy())
        ...
        dfCombined.to_csv("/mnt/raid/home/ayrisbud/us-famli-pl/src/predictedClassifierExp1.csv", index=False)
        ...
        myClassDf.to_csv("/mnt/raid/home/ayrisbud/us-famli-pl/src/classification_report/UNet/epoch=117-val_loss=0.372.csv", header=True, index=True)
        print(myClassDf)
        ...
        plt.tight_layout()
        plt.show()
        plt.savefig("/mnt/raid/home/ayrisbud/us-famli-pl/src/classification_report/UNet/epoch=117-val_loss=0.372_confMat.png")
        ...
        plt.savefig("/mnt/raid/home/ayrisbud/us-famli-pl/src/classification_report/UNet/epoch=117-val_loss=0.372_auc.png")
