...
#     overlap_mask = None
#     print("Clone: ", clone)
#     print("Input Image", type(input_image))
#     print(type(feature_maps))
    
#     for i in range(feature_maps.shape[1]):
#         grads = torch.autograd.grad(outputs=model.feature_maps[:, i].mean(), inputs=input_image, retain_graph=True, allow_unused=True)[0]
#         print("GRADS: ", grads)
#         pooled_grads = torch.mean(grads, dim=[0, 2, 3])
        
#         for j in range(feature_maps.shape[1]):
#             feature_maps[:, j, :, :] *= pooled_grads[j]
...
#             return overlap_removed_heatmaps
#         else:
#             return heatmaps
...
class EfficientNetClone(pl.LightningModule):
    def __init__(self, args = None, base_encoder='efficientnet_b0', num_classes=6, lr=1e-3, weight_decay=1e-4, class_weights=None, ckpt_path = "./"):
        ...
        self.last_conv_layer = self.convnet.features[8][0]  # Replace with actual layer
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        myGradcam = GuidedGradCam(self, self.convnet.features[8][0])
        ...
        if hasattr(self, 'clone_model'):
            ...
            myGradcamClone = GuidedGradCam(self.clone_model, self.clone_model.convnet.features[8][0])
        ...
        self.BCELoss = loss
        self.log('train_loss', combined_loss, on_epoch=True, on_step=True, sync_dist=True)
        self.log("train_acc", self.accuracy, on_epoch=True, on_step=True, sync_dist=True)
    ...
    def training_epoch_end(self, outputs):
        ...
        if (self.current_epoch + 1) % 10 == 0 and self.dice_weight <=0.5:
