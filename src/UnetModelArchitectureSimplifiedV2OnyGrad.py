...
def remove_overlap(heatmaps, labels,  threshold=0.5):
    ...
    heatmapStack = heatmapStack * labels
...
def getHeatmaps(gradcam, input_image, num_classes, labels):
    AttribsList = []
    for i in range(num_classes):
        ...
        # heatmaps = heatmaps[1:]
    toReturn = remove_overlap(AttribsList, threshold=0.5, labels)
...
class SharedBackboneUNet(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % 5 == 0 or self.global_step == 0 or x.shape != self.lastXShapeTrain:
            ...
            input_image = x#[:, 0, :, :]
            self.gradcamHeatmapsTrain = getHeatmaps(gradcam=myGradcamClone, input_image=input_image.requires_grad_(True), num_classes=self.num_classes, labels=y)
