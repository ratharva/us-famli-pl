...
class EfficientNetClone(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'clone_model'):
            myGradcam = GuidedGradCam(self.clone_model, self.clone_model.last_conv_layer)
            gradcam_heatmaps = getHeatmaps(gradcam=myGradcam, input_image=x.requires_grad_(True), num_classes=self.num_classes, clone=True)
        else:
            myGradcam = GuidedGradCam(self, self.last_conv_layer)
            gradcam_heatmaps = getHeatmaps(gradcam=myGradcam, input_image=x.requires_grad_(True), num_classes=self.num_classes, clone=False)
        

        # return torch.sigmoid(fwdPass)
        return fwdPass, gradcam_heatmaps
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if hasattr(self, 'clone_model'):
            ...
            # print(x.shape)
            with torch.no_grad():
                y_pred_clone, gradcam_heatmaps_clone = self.clone_model(x_copy.cuda())
