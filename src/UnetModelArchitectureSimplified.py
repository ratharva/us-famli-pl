...
class SharedBackboneUNet(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        x = self.encoder._conv_stem_padding(x)
        x = self.encoder._bn0(x)
        x = self.encoder._blocks[0](x)
        x = self.encoder._blocks[1](x)
        x = self.encoder._blocks[2](x)
        x = self.encoder._blocks[3](x)
        x = self.encoder._blocks[4](x)
        x = self.encoder._blocks[5](x)
        # print(x.shape)
        x = self.encoder._blocks[6](x)
        # print(x.shape)
        x = self.encoder._conv_head(x)
        x = self.encoder._conv_head_padding(x)
        x = self.encoder._bn1(x)
        
        classification = self.encoder._avg_pooling(x)
        classification = classification.view(classification.size(0), -1)
        classification = self.encoder._dropout(classification)
        classification = self.encoder._fc(classification)
