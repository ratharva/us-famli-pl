...
import cloneModelArchitecture
import cloneModelArchitectureV2
import cloneModelArchitectureV3
import cloneModelArchitectureV4
import cloneModelArchitectureV3Updated
import UnetModelArchitecture
import UnetModelArchitectureMonai
import UnetModelArchitectureSimplifiedV2
import UnetModelArchitectureSimplified
import UnetModelArchitectureSimplifiedV2Pretrained
import UnetModelArchitectureSimplifiedPretrained
...
def main(args):
    ...
    usdata = USDataModule(df_train, df_val, df_test, label_columns=myLabels, batch_size=args.batch_size, num_workers=args.num_workers, img_column=img_column, repeat_channel=False, train_transform=USClassTrainTransforms(256), valid_transform=USClassEvalTransforms(256), test_transform=USClassEvalTransforms(256))

    # print("Dataset length: ", len(usdata))
    checkpoint_callback = ModelCheckpoint(
        dirpath=args.out,
        filename='Unet_{epoch}-{val_loss:.3f}',
        save_top_k=2,
        monitor='val_loss'
    )

    # model = cloneModelArchitecture.EfficientNetClone(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    # model = cloneModelArchitectureV2.EfficientNetClone(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    # model = cloneModelArchitectureV3.EfficientNetClone(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    # model = cloneModelArchitectureV4.EfficientNetClone(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    # model = cloneModelArchitectureV3Updated.EfficientNetClone(num_classes=len(myLabels)) #, ckpt_path=args.ckpt_path
    # model = UnetModelArchitecture.SharedBackboneUNet(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    # model = UnetModelArchitectureMonai.SharedBackboneUNet(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    # model = UnetModelArchitectureSimplifiedV2.SharedBackboneUNet(num_classes=len(myLabels), ckpt_path=args.ckpt_path)   
    # model = UnetModelArchitectureSimplified.SharedBackboneUNet(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    # model = UnetModelArchitectureSimplifiedV2Pretrained.SharedBackboneUNet(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    model = UnetModelArchitectureSimplifiedPretrained.SharedBackboneUNet(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    ...
    trainer = Trainer(
        # resume_from_checkpoint='/mnt/raid/home/ayrisbud/train_output/Unet/Unet_epoch=59-val_loss=0.377.ckpt',
        # strict = False,
        logger=logger,
