...
def main(args):
    ...
    usdata = USDataModule(df_train, df_val, df_test, label_columns=myLabels, batch_size=args.batch_size, num_workers=args.num_workers, img_column=img_column, train_transform=USClassTrainTransforms(256), valid_transform=USClassEvalTransforms(256), test_transform=USClassEvalTransforms(256), repeat_channel=True)


    checkpoint_callback = ModelCheckpoint(
        dirpath=args.out,
        filename='Metric_{epoch}-{val_loss:.2f}',
        save_top_k=2,
        monitor='val_loss'
    )
    ...
    # model = multilabelModel.EfficientNetNew(num_classes=len(myLabels), ckpt_path=args.ckpt_path)
    early_stop_callback = EarlyStopping(monitor="combinedMetrics", min_delta=0.00, patience=30, verbose=True, mode="max")
